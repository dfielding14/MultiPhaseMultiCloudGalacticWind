#!/usr/bin/env python
"""
Shorter production MCMC run for galaxy J0021+0052.

This script runs a production-quality MCMC fit with:
- 32 walkers
- 2,000 steps  
- 10 parallel cores
- Full degeneracy analysis

This is a shorter but still meaningful production run.
"""
...
def run_shorter_production():
    """Run shorter production MCMC for J0021+0052."""
    
    # Target galaxy
    galaxy_name = 'J0021+0052'
    
    # Production parameters (reduced for reasonable runtime)
    n_walkers = 32
    n_steps = 2000
    n_threads = 10
    
    print("="*70)
    print("PRODUCTION MCMC RUN FOR J0021+0052 (SHORTER VERSION)")
    print("="*70)
    print(f"\nConfiguration:")
    ...
    print(f"  Expected runtime: ~{n_steps * 11 / n_threads / 60:.1f} minutes")
    print("="*70)
    ...
    # Initialize fitter with fewer cloud species for speed
    fitter = CLASSYFitter(galaxy_data, n_cloud_species=7)
    
    # Set up initial positions - use a tighter ball around reasonable values
    ndim = 3  # eta_M, eta_M_cold, eta_E
    
    # Initial guess based on test run results
    initial = np.array([0.3, 1.0, 0.1])  
    
    # Create initial walker positions with small scatter
    np.random.seed(42)  # For reproducibility
    pos = initial + 0.05 * initial[np.newaxis, :] * np.random.randn(n_walkers, ndim)
    ...
    for i in range(n_walkers):
        ...
        while not np.isfinite(log_prior(pos[i])) and attempts < 100:
            pos[i] = initial + 0.05 * initial * np.random.randn(ndim)
            attempts += 1
        if attempts >= 100:
    ...
    output_dir = 'production_results_shorter'
    ...
    n_burn = min(500, sampler.iteration // 4)
    ...
    print("\n" + "-"*70)
    print("EVALUATING BEST-FIT MODEL")
    print("-"*70)
    
    # Run model with best-fit parameters
    best_eta_M = results['eta_M']['median']
    best_eta_M_cold = results['eta_M_cold']['median']
    best_eta_E = results['eta_E']['median']
    
    print(f"\nRunning model with best-fit parameters...")
    model_results = fitter.evaluate_model(
        best_eta_M, best_eta_M_cold, best_eta_E,
        galaxy_data['sfr'], galaxy_data['v_circ'], galaxy_data['r50']
    )
    
    if model_results[0] is not None:
        log_v_model, log_width_model, log_NH_model = model_results
        print(f"\nModel predictions vs Observations:")
        print(f"  log(v):     {log_v_model:.2f} vs {galaxy_data['log_v']:.2f} (obs)")
        print(f"  log(width): {log_width_model:.2f} vs {galaxy_data['log_width']:.2f} (obs)")
        print(f"  log(NH):    {log_NH_model:.2f} vs {galaxy_data['log_NH']:.2f} (obs)")
        
        # Calculate chi-squared
        chi2 = 0
        chi2 += ((log_v_model - galaxy_data['log_v']) / galaxy_data['log_v_err'])**2
        chi2 += ((log_width_model - galaxy_data['log_width']) / galaxy_data['log_width_err'])**2
        chi2 += ((log_NH_model - galaxy_data['log_NH']) / galaxy_data['log_NH_err'])**2
        print(f"\nChi-squared: {chi2:.2f}")
        print(f"Reduced chi-squared: {chi2/3:.2f}")
    else:
        print("  ⚠ Could not evaluate best-fit model")
...
if __name__ == "__main__":
    sampler, results, deg_results = run_shorter_production()