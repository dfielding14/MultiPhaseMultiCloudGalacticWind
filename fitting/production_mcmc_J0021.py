#!/usr/bin/env python
"""
Production MCMC fit for CLASSY galaxy J0021+0052

This script performs a full Bayesian parameter inference for the
multiphase wind model using observational data from the CLASSY survey.
"""
...
GALAXY_DATA = {
    ...
    'sfr_err': 0.5,      # Msun/yr
    'v_circ': 69.0,      # km/s (from stellar velocity dispersion)
...
# Velocity bins and column density per velocity bin
SiII_DATA = {
...
def create_wind_config():
    ...
    return WindConfig(
        N_cloud_species=7,        # Moderate number for balance of accuracy/speed
...
def run_wind_model(theta, config):
    ...
    try:
        ...
        print(f"Model failed with theta={theta}: {e}")
        return None, None
...
def log_likelihood(theta, config, use_both_ions=True):
    ...
    config : WindConfig
        Model configuration
    use_both_ions : bool
        If True, fit both Si II and C II; if False, only Si II
    """
    # Run model
    ...
    if use_both_ions:
        # C II likelihood (with scaling factor since model gives total)
        # In reality, different ions trace different cloud populations
        # For simplicity, we assume C II traces 80% of clouds
        CII_scaling = 0.8
...
def log_prior(theta):
    ...
    We use physically motivated bounds based on simulations
    and observations from the literature.
    """
    log_eta_M, log_eta_M_cold, log_eta_E = theta
    
    # Physical bounds (in log10 space) - adjusted to avoid numerical issues
    if -1.5 < log_eta_M < 0.5:         # 0.03 to ~3
        if -1.0 < log_eta_M_cold < 1.0:  # 0.1 to 10
            if -1.5 < log_eta_E < 0.7:   # 0.03 to ~5
                # Avoid problematic region where cold >> hot
                eta_M = 10**log_eta_M
                eta_M_cold = 10**log_eta_M_cold
                if eta_M_cold > 20 * eta_M:  # Avoid extreme ratios
                    return -np.inf
...
def run_mcmc(nwalkers=64, nsteps=5000, nburn=1000, nthreads=None):
    """
    Run production MCMC.
    ...
    ----------
    nwalkers : int
        Number of walkers (should be >> ndim)
    nsteps : int
        Number of production steps
    nburn : int
        Number of burn-in steps
    nthreads : int or None
        Number of parallel threads (None for serial)
    """
    print("=" * 70)
    print("PRODUCTION MCMC FIT FOR J0021+0052")
    print("=" * 70)
    ...
    # Initialize walkers
    # Start near reasonable values based on simulations
    initial_guess = np.array([-0.7, -0.3, -0.2])  # log10([0.2, 0.5, 0.6])
    initial_scatter = 0.15  # Smaller scatter for better stability
...
def analyze_results(sampler, make_plots=True):
    ...
    if v_model is not None:
        ...
        N_interp_SiII = np.interp(SiII_DATA['v'], v_model, dN_dv_model)
        chi2_SiII = np.sum(((SiII_DATA['N'] - N_interp_SiII) / SiII_DATA['N_err'])**2)
        dof_SiII = len(SiII_DATA['v']) - 3  # data points - parameters
...
def create_plots(sampler, samples, theta_best, config):
    ...
    plt.suptitle('MCMC Chains for J0021+0052', fontsize=14)
    plt.tight_layout()
    plt.savefig('mcmc_chains_J0021.pdf', dpi=150, bbox_inches='tight')
    plt.show()
    ...
    fig.suptitle('Parameter Posterior for J0021+0052', fontsize=14, y=1.02)
    plt.savefig('mcmc_corner_J0021.pdf', dpi=150, bbox_inches='tight')
    plt.show()
    ...
    if v_model is not None:
        ...
        N_interp = np.interp(SiII_DATA['v'], v_model, dN_dv_model)
        ...
        plt.suptitle('J0021+0052: Model vs Observations', fontsize=14)
        plt.tight_layout()
        plt.savefig('mcmc_fit_J0021.pdf', dpi=150, bbox_inches='tight')
        plt.show()
...
if __name__ == "__main__":
    # Run production MCMC
    # Production run with 10 cores as requested
    
    sampler = run_mcmc(
        nwalkers=100,    # 100 walkers for good sampling
        nsteps=2000,     # 2000 production steps
        nburn=500,       # 500 burn-in steps
        nthreads=10      # Use 10 cores as requested
    )
    ...
    with open('mcmc_results_J0021.pkl', 'wb') as f:
        pickle.dump(results, f)
    
    print("\nResults saved to mcmc_results_J0021.pkl")
