...
# cold_cloud_injection_radial_power   = 2
# cold_cloud_injection_radial_extent  = 1.1001*r0
# cloud_radial_offest                 = 1e-1
...
plt.semilogy(v_cloud[:-50]/1e5, cloud_density[:-50]/(mu*mp) / (np.gradient(v_cloud[:-50],r[:-50]) / 1e5))
plt.ylabel(r'$dN/dv$ [cm$^{-2}$ (km/s)$^{-1}$]')
plt.xlabel(r'$v$ [km/s]')
...
fig,((axv,axZ),(axM,axMd)) = plt.subplots(2,2,sharex=True,constrained_layout=True)
...
axv.loglog(r/kpc, v_wind/1e5,                                                        color = 'tab:blue' )
axv.loglog(r/kpc, cs_wind/1e5,                                                       color = 'tab:blue' ,ls = cs_linestyle )
axv.loglog(r/kpc, np.ma.masked_where(M_cloud<M_cloud_min, v_cloud)/1e5,        color = 'tab:blue' ,ls = cloud_linestyle )
axZ.loglog(r/kpc, rhoZ_wind / rho_wind / Z_solar,                              color = 'tab:blue' )
axZ.loglog(r/kpc, np.ma.masked_where(M_cloud<M_cloud_min, Z_cloud)/Z_solar,    color = 'tab:blue' ,ls = cloud_linestyle )
axM.loglog(r/kpc, np.ma.masked_where(M_cloud<M_cloud_min, M_cloud)/Msun,       color = 'tab:blue' )
if sol.status == 1:
    axM.scatter(r[-1]/kpc, np.ma.masked_where(M_cloud<M_cloud_min, M_cloud)[-1]/Msun,  color = 'tab:blue' , marker='x', s=8)
axMd.loglog(r/kpc, Mdot_wind,                                                       color = 'tab:blue' )
axMd.loglog(r/kpc, np.ma.masked_where(M_cloud<M_cloud_min, cloud_Mdots[0]),  color = 'tab:blue' , ls = cloud_linestyle)
...
custom_lines = [Line2D([0], [0], color='k', ls = '-'),
                Line2D([0], [0], color='k', ls = cloud_linestyle)]
axMd.legend(custom_lines, [r'${\rm hot}$', r'${\rm cold}$'],loc='lower right',fontsize=6, frameon=False, handlelength=2.2, labelspacing=0.3)
...
fig.suptitle(r'$ {\rm SFR} = '+str(int(SFR/(Msun/yr)))+r' \ M_\odot / {\rm yr} \quad \eta_{\rm M, hot} = '+str(np.round(eta_M,2))+r'\quad \eta_{\rm M, cold} = '+str(np.round(eta_M_cold_tot,2))+r'$',fontsize=10)

fig.set_size_inches(5,4)
plt.savefig('GroupMeeting_Example_SFR'+str(int(SFR/(Msun/yr)))+'_etaE1_etaM'+str(np.round(eta_M,2))+'_etaMcold'+str(np.round(eta_M_cold_tot,2))+'_vcl3e1_Zcl1_distributed_injection_to_1+'+str(cloud_radial_offest)+'_'+str(np.round(cold_cloud_injection_radial_extent/r0,3))+'r0_power'+str(cold_cloud_injection_radial_power)+'_logMcl_'+str(log_M_cloud0)+'_vcirc'+str(np.round(v_circ0/1e5))+'_cooling_split_perfect.pdf',dpi=200,bbox_inches='tight')


cloud_density = Ndot_cloud0 *  np.where(r<cold_cloud_injection_radial_extent, (r/cold_cloud_injection_radial_extent)**cold_cloud_injection_radial_power, 1.0) * M_cloud / (Omwind * r**2 * v_cloud)

Column_density = np.sum(np.gradient(r)*cloud_density)/(mu*mp)
v_out = np.average(v_cloud, weights=np.log10(cloud_density))
r_out = r[np.argmin(np.abs(np.cumsum(np.gradient(r)*cloud_density)/(mu*mp) - Column_density/2.0))]

Mdot_estimate = 4*np.pi * r_out * v_out * Column_density * mu*mp / (Msun/yr)

axv.axhline(v_out/1e5, lw=5, alpha=0.5, color='k')
axMd.axhline(Mdot_estimate, lw=5, alpha=0.5, color='k')
axM.text(0.05, 0.05, r'$N='+str(np.round(10**(np.log10(Column_density) - np.floor(np.log10(Column_density))),1))+r'\cdot 10^{'+str(int(np.floor(np.log10(Column_density))))+r'} \ {\rm cm}^2$', horizontalalignment='left',  verticalalignment='bottom', transform=axM.transAxes)
plt.show()
...
plt.loglog(r/kpc, np.cumsum(np.gradient(r)*cloud_density)/(mu*mp))
plt.ylim(1e18,1e21)
plt.axvline(r_out/kpc, color='k', linestyle='--')
plt.ylabel(r'$N(r)$ (cm$^{-2}$)')
plt.xlabel(r'$r$ (kpc)')
...
plt.semilogy(-v_cloud/1e5, cloud_density/(mu*mp) / (np.gradient(v_cloud,r) / 1e5))
plt.ylabel(r'$dN/dv$ [cm$^{-2}$ (km/s)$^{-1}$]')
plt.xlabel(r'$v$ [km/s]')
# plt.ylim(top=1e19)
...
plt.loglog(v_cloud/1e5, (np.gradient(r)*cloud_density)/(mu*mp))
plt.ylim(1e14,1e21)
plt.axvline(v_out/1e5, color='k', linestyle='--')
plt.ylabel(r'$N(r)$ (cm$^{-2}$)')
plt.xlabel(r'$v_{cloud}$ (km/s)')
