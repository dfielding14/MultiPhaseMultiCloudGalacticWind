...
area scalings.  After the first pass, we also include the restricted effective
coordinates motivated by the screen itself: an amplitude $A_{\rm mix}$ that
multiplies the cloud mass-exchange terms, and a diagnostic density-contrast
tilt $\beta_{\chi,\rm mix}$ in
$A_{\rm mix}(\chi/100)^{\beta_{\chi,\rm mix}}$.  These are still treated as
fixed forward-model perturbations in this section, not as fitted parameters.
The same reduced CPU settings used in the exact recovery tests are used here:
$r_{\rm max}=6\,{\rm kpc}$, $\Delta r=0.08\,{\rm kpc}$, four cloud-mass
species, and cloud masses from $10$ to $10^4\,M_\odot$.
...
mostly loading-like, risky \\
$\beta_{\chi,\rm mix}$ & 0.63 & 91.7 & 0.87 & 0.979 & $1.7\times10^3$ &
diagnostic chi tilt, risky \\
$A_{\rm area}$ & 0.56 & 84.9 & 0.90 & 0.998 & $1.6\times10^3$ &
mostly loading-like, risky \\
$A_{\rm mix}$ & 0.34 & 74.9 & 0.90 & 0.998 & $1.7\times10^3$ &
restricted amplitude candidate \\
$A_{\dot M}$ & 0.34 & 74.9 & 0.90 & 0.998 & $1.7\times10^3$ &
direct amplitude proxy \\
...
parameters included in the profile screen.  $A_{\rm mix}$ and $A_{\dot M}$
are identical in this screen because both multiply the same mass-exchange terms
when $\beta_{\chi,\rm mix}=0$.  ``Min. valid'' is the minimum valid fraction
across the staged scans.  ``Load frac.'' is the maximum fraction of the
...
exponents, from the diagnostic $\beta_{\chi,\rm mix}$ tilt, and from effective
mixing-amplitude parameters such as $A_{\rm mix}$, the mass-exchange prefactor,
geometric area factor, and $f_{\rm turb}$.  These large shifts demonstrate that
the microphysics matters for the forward model, but they do not by themselves
establish that the parameters are independently inferable.
}
...
profile by tens of likelihood-scale units.  The direct chi-tilt
$\beta_{\chi,\rm mix}$ is also high leverage, which is scientifically
encouraging but not yet an identifiability result.  The effective
amplitude-like parameters, especially $A_{\rm mix}$, the mass-exchange
prefactor, geometric area factor, and $f_{\rm turb}$, also move both the
compact summaries and the profile shape.  By contrast, the global hot cooling
multiplier is nearly inert in this reduced screen, and the metallicity
variations have lower leverage than the mixing and turbulence parameters.
...
mass-exchange prefactor, $A_{\rm mix}$, or $f_{\rm turb}$ can stall
...
prefactor, geometric area factor, $A_{\rm mix}$, and $f_{\rm turb}$ move the
observed-window profile in the fiducial and low-cold-loading cases as well as
in the high-energy cases.  The $\chi$-dependent exponents and
$\beta_{\chi,\rm mix}$ are even more global: one sign of the perturbation can
suppress mixing enough to remove the wings, while the other can over-couple the
phases and stall the wind.  Physically, these parameters regulate how rapidly
cold material exchanges mass, momentum, and energy with the hot flow.  When the
coupling is too strong, the hot phase can lose enough specific energy to fail
before $6\,{\rm kpc}$; when it is weaker, the cool material survives to
different velocities and changes the line-profile shape.  That is exactly the
microphysics we eventually want to understand, but it is not yet a stable
fourth or fifth inference coordinate.
...
later, the most physically controlled first candidate is the effective mixing
amplitude $A_{\rm mix}$.  In this screen it reproduces the old
mass-exchange-prefactor response, which is the intended behavior: it isolates
the amplitude of the cloud mass-exchange terms without fitting every closure
coefficient separately.  Even that candidate should use a restricted prior,
because the high-amplitude end of the scan produces stalled winds in the
high-energy cases.  The drag coefficient is validity-stable and has measurable
profile leverage, but its overall response is smaller than the
mixing-amplitude directions and it is lower priority as a first expansion.  The
$\beta_{\chi,\rm mix}$ direction is scientifically important because it probes
the density-contrast dependence directly, but it should remain a second-stage
diagnostic until the $A_{\rm mix}$-only recovery gate passes.

The same caution applies to the assumed cloud mass distribution.  The screen
does include the mass-spectrum slope $\alpha_{\rm cl}$, and it has measurable
leverage: it changes the observed-window profile while remaining largely
parallel to the loading subspace.  This is physically sensible.  A steeper
initial spectrum puts more weight in small clouds, which mix, slow, and
disrupt more readily; a shallower spectrum puts more weight in massive clouds
that survive farther and can carry column to different velocities.  These
effects can mimic changes in $\eta_{M,\rm cold}$ or in the effective mixing
amplitude.  The adopted cloud mass limits add another layer of caution.  With
only four cloud-mass species, changing the lower or upper mass cutoff changes
both the physical population and the quadrature used to represent it.  We
therefore treat the fiducial $10$--$10^4\,M_\odot$ range and
$\alpha_{\rm cl}=2$ as fixed assumptions for the next validation gate, not as
new inference parameters.  Before making production claims with an expanded
$A_{\rm mix}$ model, the recovery should be repeated or stress-tested over a
small grid of reasonable cloud mass ranges and slopes.
...
predictive and synthetic-recovery gate while remaining robust to reasonable
cloud-population choices.  None of those conditions is met by the present
screen.

We therefore stop short of treating any new coordinate as a production
parameter at this stage.  The
...
support fitting a broad set of TRML closure knobs.  The next validation stage
should focus on a very restricted $A_{\rm mix}$ experiment with its own prior
predictive and synthetic-recovery gate, rather than a general expansion of the
closure model.  Only after that gate passes should the
$\beta_{\chi,\rm mix}$ density-contrast diagnostic be interpreted as an
inference target.
